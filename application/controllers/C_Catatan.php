...
class C_Catatan extends CI_Controller {
	...
	public function MasukanNoRubrik(){
		...
		$jenis=$this->input->post('jenis_surat');
			...
			$nomer=$this->M_Catatan->cek_nomer_terakhir();
			$no='';
        ...
        header('Location: '.base_url().'index.php/C_Catatan/TampilkanNomerRubrik_surat1/'.$surat);
		}
		else{
		$data['success']=0;
		$data['active']='rubrik';
	...
	public function TampilkanNomerRubrik_surat1($noSurat){ //menampilkan tabel dengan case input terlebih dahulu
		...
		$hasil = $this->M_Catatan->lihat_rubrik();
	...
	public function EditNoSurat(){
		...
		$array = array('nomor_catatan' => $noSurat, 'penandatangan'=>$penandatangan, 'perihal'=>$perihal, 'dari'=>$dari );
		$hasil = $this->M_Catatan->update_catatan($id, $array);
		
		if($hasil==1){
			$noSurat=explode('/',$noSurat);
			$surat=implode('_',$noSurat);
         header('Location: '.base_url().'index.php/C_Catatan/TampilkanNomerRubrik_surat3/'.$surat);
		}
		else{
		    $noSurat=explode('/',$noSurat);
			$surat=implode('_',$noSurat);
         header('Location: '.base_url().'index.php/C_Catatan/TampilkanNomerRubrik_surat4/'.$surat);
	...
	public function MasukanBackdate_Surat(){
		...
		$jenis=$this->input->post('jenis_surat');
		$noSurat='';
			$nomer=$this->M_Catatan->cek_backdate($tgl);
