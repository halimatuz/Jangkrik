...
class C_Memo extends CI_Controller {
	...
	public function MasukanNoRubrik(){
		...
		$jenis=$this->input->post('jenis_surat');
			...
			$nomer=$this->M_Memo->cek_nomer_terakhir();
			$no='';
        ...
        header('Location: '.base_url().'index.php/C_Memo/TampilkanNomerRubrik_surat1/'.$surat);
		}
		else{
		$data['success']=0;
		$data['active']='rubrik';
	...
	public function TampilEditNoSurat($noSurat){
     $data['success']=1;
		...
		$hasil = $this->M_Memo->cek_memo($NewSurat);
		$data['kepada']='';
		$data['perihal']='';
		$data['tanggal']='';
		$data['id']='';
		...
		$this->load->view('V_Asidebar',$data);
		$this->load->view('V_Top_Anchor',$data);
		$this->load->view('V_EditNomorRubrik_Memo',$data);
		$this->load->view('V_Footer');
	...
	public function EditNoSurat(){
		...
		if($hasil==1){
			...
			$surat=implode('-',$noSurat);
			echo $surat;
         header('Location: '.base_url().'index.php/C_Memo/TampilkanNomerRubrik_surat3/'.$surat);
		}
		else{
		    $noSurat=explode('/',$NewSurat);
			$surat=implode('-',$noSurat);
			echo $surat;
         header('Location: '.base_url().'index.php/C_Memo/TampilkanNomerRubrik_surat4/'.$surat);
	...
	public function TampilMasukanBackdate(){
		...
		$this->load->view('V_Asidebar',$data);
		$this->load->view('V_Top_Anchor',$data);
		$this->load->view('V_MasukanBackdateMemo',$data);
		$this->load->view('V_Footer');
	}
	public function MasukanBackdate_Surat(){
		...
		$jenis=$this->input->post('jenis_surat');
			...
			$nomer=$this->M_Memo->cek_backdate($tgl);
